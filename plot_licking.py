...
for mouse_name in mice:
    for TT in trialtypes:
        fig,ax = plt.subplots(1,5,sharex = True, sharey = True,figsize = (30,10))
        ...
        for index,i in enumerate(cond_days):
            ...
            ax[0].plot(licking_conv+gap*index,color =cm.cool(index/float(5)),label = all_days[i])
            ax[0].legend(frameon=False)
            ax[0].set_ylim(0,gap*5+10)
            ax[0].set_title('conditioning',fontsize = 20)
            ax[0].axvline(x = odor_on_point,color = 'k')
            ax[0].axvline(x = odor_off_point,color = 'k')
            ax[0].axvline(x = water_point)
            
        for index,i in enumerate(deg_days):
            licking_conv = np.convolve(basis,across_day_licking[:,i],mode = 'full')[0:sample_points]
            ax[1].plot(licking_conv+gap*index,color =cm.cool(index/float(5)),label = all_days[i])
            ax[1].legend(frameon=False)
            ax[1].set_title('degradation',fontsize = 20)
            ax[1].axvline(x = odor_on_point,color = 'k')
            ax[1].axvline(x = odor_off_point,color = 'k')
            ax[1].axvline(x = water_point)
        for index,i in enumerate(rec_days):
            licking_conv = np.convolve(basis,across_day_licking[:,i],mode = 'full')[0:sample_points]
            ax[2].plot(licking_conv+gap*index,color =cm.cool(index/float(5)),label = all_days[i])
            ax[2].legend(frameon=False)
            ax[2].set_title('recovery',fontsize = 20)
            ax[2].axvline(x = odor_on_point,color = 'k')
        ...
        for index,i in enumerate(ext_days):
            ...
            ax[3].axvline(x = odor_on_point,color = 'k')
            ax[3].axvline(x = odor_off_point,color = 'k')
            ax[3].axvline(x = water_point)
        for index,i in enumerate(finalrec_days):
            licking_conv = np.convolve(basis,across_day_licking[:,i],mode = 'full')[0:sample_points]
            ax[4].plot(licking_conv+gap*index,color =cm.cool(index/float(5)),label = all_days[i])
            ax[4].legend(frameon=False)
            ax[4].set_title('final recovery',fontsize = 20)
            ax[4].axvline(x = odor_on_point,color = 'k')
            ax[4].axvline(x = odor_off_point,color = 'k')
