function twochannel_BehProtocol_MSFP_Synching_Resaving(filedir, filename,DoricStudioVersion)
    ...
    if ~isempty(doric_file)
        ...
        % Because the Doric software names the first ROI as ROI0. Mine is 0,2,3,4,5,6,7,8,9,10,10,11
        if num_CAM == 1
            for i_column = 1:num_var
                if i_column == 6|i_column == 7
                ...
                elseif i_column == 26 | i_column == 27
                    aux = strjoin({names1{i_column}(1:min(length(names1{i_column}),6)),'_',names2{i_column}});
                    var_names{i_column} = strrep(aux(~isspace(aux)),'10','11');
                elseif i_column == 28 | i_column == 29
                    aux = strjoin({names1{i_column}(1:min(length(names1{i_column}),6)),'_',names2{i_column}});
        ...
        figure
        for i_ROI = 1:num_ROI
            ind = strfind(var_names, ['ROI' num2str(i_ROI)]);  
            Index = find(not(cellfun('isempty',ind)));
    ...
    %     for i_trial = 1:num_TTL
        ...
        for i_exc = 1:num_EXC                     
            ...
            for i_trial = 1:num_TTL
                ...
                for i_roi = 1:num_ROI
                    index_roiexc = strfind(var_names, ['ROI' num2str(i_roi)]); 
