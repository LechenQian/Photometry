function threechannel_BehProtocol_MSFP_Synching_Resaving(filedir, filename,DoricStudioVersion)
    ...
    if ~isempty(doric_file)
        ...
        for i_column = 1:num_var
            ...
            elseif i_column == 31 | i_column == 32 | i_column == 33
                aux = strjoin({names1{i_column}(1:min(length(names1{i_column}),6)),'_',names2{i_column}});
                var_names{i_column} = strrep(aux(~isspace(aux)),'10','11');
            elseif i_column == 34 | i_column == 35 | i_column == 36
                aux = strjoin({names1{i_column}(1:min(length(names1{i_column}),6)),'_',names2{i_column}});
        ...
        figure
        for i_ROI = 1:num_ROI
            ind = strfind(var_names, ['ROI' num2str(i_ROI)]);  % Because the Doric software names the first ROI as ROI0
            Index = find(not(cellfun('isempty',ind)));
